...
```{r, eval = FALSE}
impute_pop <-
  po("imputesample") %>>% po("imputemedian") %>>% po("modelmatrix", formula = ~ .  - 1)

impute_pop$train(tasks[["d7"]])[[1]]
```
...
Let´s then define the grid for the benchmark. Again, we chose 5-fold cross-validation.
...
bmr$learners$learner[[3]]
```


Since nothing else is working, let´s work on that pipeline for IBS:
```{r}
pipe_IBS <- ppl(
  "distrcompositor",
...
train_IBS <- pipe_IBS$train(tasks[["d3"]])
train_IBS
```
compose_distr. has no solution: NULL 
